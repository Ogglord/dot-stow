...
#
# ARG_POSITIONAL_SINGLE([command],[Which command to execute],[])
# ARG_OPTIONAL_BOOLEAN([dry-run],[n],[Do not write changes to disk/git])
# ARG_OPTIONAL_BOOLEAN([force],[f],[Force changes, overwrite existing files])
# ARG_OPTIONAL_SINGLE([package],[p],[Specify stow package])
# ARG_POSITIONAL_INF([arg],[Files/Directories or Packages to go process],[0])
# ARG_HELP([Helps move, and stow dotfiles using GNU Stow and push/pull changes to git])
...
## print with left and right column
msg2() {
